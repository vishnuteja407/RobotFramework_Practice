...
Variables   TestData/Setup_details.yaml
Variables   TestData/cewa_variables.yaml
...
#    Execute Process Templates       TestData/process-template.json      ${ServiceCatalog.order_details.power_supply.expected_fail_command}
...
#    Execute Process Templates       TestData/process-template.json      ${ServiceCatalog.order_details.platform_validation.expected_fail_command}
    ...
    Execute Device Auth     TestData/device-auth.json
...
#        ${arr['domain']}=    Convert To Upper Case    ${arr['domain']}
    ...
    FOR    ${i}     IN RANGE     ${count}
       run keyword and continue on failure   dictionaries should be equal	${Sorted_expected_array}[${i}]     ${Sorted_host_domain_array}[${i}]  	ignore_order=True
    END
